...
3. If there's an error, set the error state to `'Username must be lower case'`.
   ...
   `setError(isValid ? null : 'Username must be lower case')`) and disable the submit button.
...
of the form inputs have been "uncontrolled" which means that the
...
control the input value. Anytime there's a change we'll call `.toLowerCase()`
...
<a rel="noopener noreferrer" target="_blank" href="https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=alex.king%40deepcrawl.com">
